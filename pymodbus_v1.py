...
client = ModbusSerialClient(method='rtu', port='COM10', baudrate=9600, stopbits=1, bytesize=8, parity='N')
client.connect()

try:
    while True:
        # Read the holding register value from the ESP8266
        response = client.read_holding_registers(address=3019, count=2, unit=1)
        if response.isError():
